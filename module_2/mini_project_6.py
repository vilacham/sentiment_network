...
class SentimentNetwork:
    def __init__(self, reviews, labels, min_count=10, polarity_cutoff=0.1,
        ...
        given reviews
        min_count (int): Minimum occurrence of a word in the vocabulary in
        order to add it to the review vocab
        polarity_cutoff (float): Minimum value of positive to negative ratio
        for a word to be in review vocabulary
    ...
    def pre_process_data(self, reviews, labels, min_count, polarity_cutoff):
        ...
        for a word to be in review vocabulary
        """
        # Calculate positive-to-negative ratios of words used in the reviews
        ...
        # Populate review vocab with words that occur at least min_count times
        # in vocabulary and has absolute value of their positive to negative
        # greater than or equal polarity_cutoff
        for review in reviews:
            for word in review.split(' '):
                if total_counts[word] > min_count:
                    if word in pos_neg_ratios.keys() and \
                            abs(pos_neg_ratios[word]) >= polarity_cutoff:
